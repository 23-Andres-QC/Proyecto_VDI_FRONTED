<template>
  <div class="importar-revistas-bg-flotante">
    <div class="importar-revistas-flotante" :class="{ expanded: tabla && tabla.length > 0 }">
      <!-- Header mejorado con diseño formal -->
      <div class="importar-header-formal">
        <div class="header-content">
          <q-icon name="library_books" size="32px" class="header-icon" />
          <div class="header-text">
            <h2 class="importar-titulo-formal">Importación de Revistas Académicas</h2>
      ...
      <div class="importar-instruccion-formal">
        ...
        <div class="instruccion-text">
          <p class="instruccion-principal">
            Seleccione un archivo Excel (.xlsx) para importar revistas académicas
          </p>
      ...
      >
        <div class="visualizacion-header-formal">
          <div class="header-info">
            ...
            <div class="header-details">
              <h3 class="table-title">Visualización de Revistas Académicas</h3>
        ...
        <div class="controles-columnas">
          ...
          <div class="controles-botones">
            <q-btn
              v-if="!mostrarTodasColumnas && columnasVisibles > 10"
...
const columnasVisibles = ref(10) // Mostrar solo las primeras 10 columnas inicialmente
...
const opcionesFilasPorPagina = computed(() => {
  console.log('🔄 Calculando opciones para selector...')
  const opciones = []
  ...
  if (tabla.value && tabla.value.length > 0) {
    ...
    console.log('📊 Opciones calculadas:', opcionesUnicas)
    return opcionesUnicas
  }

  // Si no hay datos, mostrar opciones básicas
  const opcionesDefault = [...opcionesBase, 'Todas']
  console.log('📊 Opciones por defecto (sin datos):', opcionesDefault)
...
const filasParaMostrar = computed(() => {
  ...
  console.log('🔍 Estado actual:', {
    ...
    tipoSelectorFilas: typeof selectorFilas.value,
...
function mostrarMasColumnas() {
  if (columnasVisibles.value < columnas.length) {
    columnasVisibles.value = Math.min(columnasVisibles.value + 10, columnas.length)
  }
}

function mostrarMenosColumnas() {
  if (columnasVisibles.value > 10) {
    columnasVisibles.value = Math.max(columnasVisibles.value - 10, 10)
  }
}
...
function onFileChange(file) {
  ...
  reader.onload = (e) => {
    try {
      ...
      console.log('📋 Estado después de cargar archivo:', {
        totalFilas: tabla.value.length,
        paginacion: pagination.value,
        selectorFilas: selectorFilas.value,
        opciones: opcionesFilasPorPagina.value,
      })
...
onMounted(() => {
  checkAuth()
  // Asegurar que el selector se inicialice correctamente
  console.log('🚀 Componente montado, selectorFilas inicial:', selectorFilas.value)
  console.log('🚀 Opciones disponibles al montar:', opcionesFilasPorPagina.value)
})
...
/* Estado de carga formal mejorado */
.loading-section {
...
/* Nuevo estado de carga para Excel */
...
/* Clase expanded para cuando hay tabla */
.importar-revistas-flotante.expanded {
  max-width: 1600px !important;
  width: 98% !important;
}

/* Mejoras en header de tabla */
.table-actions {
  display: flex !important;
  gap: 12px !important;
  align-items: center !important;
...
.tabla-excel-formal .q-table thead th:first-child {
...
.tabla-excel-formal .q-table tbody td:first-child {
  position: sticky !important;
  left: 0 !important;
  z-index: 4 !important;
  background: #ffffff !important;
...
.tabla-excel-formal .q-table tbody tr:nth-child(even) td:first-child {
  background: #f8f9fa !important;
}

.tabla-excel-formal .q-table tbody tr:hover td:first-child {
  background: #e3f2fd !important;
}
...
/* Paginación mejorada */
.tabla-excel-formal .q-table__bottom {
  background: linear-gradient(135deg, #f8f9fa, #ffffff) !important;
...
.tabla-excel-formal .q-table__bottom .q-table__control {
  color: #1565c0 !important;
  font-weight: 600 !important;
}

/* Estados de filas */
.tabla-excel-formal .q-table tbody tr {
  transition: all 0.3s ease !important;
}
...
.tabla-excel-formal .q-inner-loading {
  background: rgba(248, 249, 250, 0.95) !important;
  backdrop-filter: blur(4px) !important;
}

/* Chips en celdas */
.tabla-excel-formal .q-chip {
  font-weight: 600 !important;
  letter-spacing: 0.5px !important;
}
...
.paginacion-controles-integrada {
  display: flex !important;
  align-items: center !important;
  gap: 12px !important;
  flex-wrap: wrap !important;
...
.selector-filas-integrado .q-field__control {
  background: #ffffff !important;
}
...
.nav-btn-integrado:hover:not(.disabled) {
  background: #1976d2 !important;
  color: white !important;
  transform: scale(1.05) !important;
}

.nav-btn-integrado.disabled {
  opacity: 0.4 !important;
  cursor: not-allowed !important;
}

.paginacion-pagina-integrada {
  font-weight: 700 !important;
  color: #1565c0 !important;
  font-size: 14px !important;
...
/* Responsive para paginación integrada */
@media (max-width: 768px) {
  .paginacion-integrada {
    flex-direction: column !important;
    gap: 12px !important;
